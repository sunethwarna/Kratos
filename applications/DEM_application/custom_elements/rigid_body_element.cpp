...
namespace Kratos {
    ...
    void RigidBodyElement::UpdateLinearDisplacementAndVelocityOfNodes() {
        ...
        for (unsigned int i = 0; i < mListOfCoordinates.size(); i++) {
            ...
            //Node<3>& sphere_node = mListOfCoordinates[i];
            array_1d<double, 3>& sphere_position = mListOfCoordinates[i]; //sphere_node.Coordinates();
            //
            // this->GetGeometry()[i] hecho con pinzas!!!!!!!
            array_1d<double, 3>& delta_displacement = this->GetGeometry()[i].FastGetSolutionStepValue(DELTA_DISPLACEMENT); //sphere_node.FastGetSolutionStepValue(DELTA_DISPLACEMENT);
            //
            ...
            noalias(this->GetGeometry()[i].FastGetSolutionStepValue(VELOCITY)) = cluster_velocity; //noalias(sphere_node.FastGetSolutionStepValue(VELOCITY)) = cluster_velocity;
        }        
    }    
    ...
    void RigidBodyElement::UpdatePositionOfNodes() {
        ...
        for (unsigned int i = 0; i < mListOfCoordinates.size(); i++) {
            ...
            noalias(this->GetGeometry()[i].FastGetSolutionStepValue(ANGULAR_VELOCITY)) = cluster_angular_velocity;
            noalias(this->GetGeometry()[i].FastGetSolutionStepValue(DELTA_ROTATION)) = cluster_delta_rotation;
        }                        
    }   
    ...
    void RigidBodyElement::CollectForcesAndTorquesFromNodes() {
        ...
        for (unsigned int i=0; i<mListOfCoordinates.size(); i++) {
            ...
            array_1d<double, 3>& particle_forces       = this->GetGeometry()[i].FastGetSolutionStepValue(TOTAL_FORCES);
            ...
            array_1d<double, 3>& sphere_position = mListOfCoordinates[i];
    ...
    void RigidBodyElement::GetRigidBodyElementsForce(const array_1d<double,3>& gravity) {
    ...
    void RigidBodyElement::SetInitialConditionsToNodes(const array_1d<double,3>& velocity) {
        for (unsigned int i=0; i<mListOfCoordinates.size(); i++) {

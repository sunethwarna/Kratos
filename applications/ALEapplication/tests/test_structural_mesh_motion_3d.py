...
class TestCase(KratosUnittest.TestCase):
    ...
    def test_Tetrahedra_3D4N(self):
        with ControlledExecutionScope(os.path.dirname(os.path.realpath(__file__))):
            test = self.createTest('test_structural_mesh_motion_3d/tetrahedra_3D4N_test')
            test.Solve()
            # remove files
            self.removeFile("./test_structural_mesh_motion_3d/tetrahedra_3D4N_test.time")
            self.removeFile("./test_structural_mesh_motion_3d/tetrahedra_3D4N_test_probe1.dat")
            self.removeFile("./test_structural_mesh_motion_3d/tetrahedra_3D4N_test_probe2.dat")
